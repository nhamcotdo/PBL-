...
static esp_err_t stop_handler(httpd_req_t *req){
    ...
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=video_file.jpg");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    videoFile.close();
    delay(100);
    videoFile = SD_MMC.open(fileName, FILE_READ);
    if(videoFile && fileName != ""){
      Serial.printf("####################### video_name: %s", videoFile.name());
      // Get the size of the video file
      size_t videoFileSize = videoFile.size();
      char* videoFileData = new char[videoFileSize];
      videoFile.readBytes(videoFileData, videoFileSize);
      videoFile.close();
      fileName = "";
...
static esp_err_t recorder_handler(httpd_req_t *req){
   Serial.printf("Start Recorder");
    fileName = "/train/video" + String(millis()) + ".avi";
...
void startCameraServer(int hPort, int sPort){
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &recorder_uri);
    }
